...
var gulp = require('gulp'),
    runSequence = require('run-sequence'),               // gulp任务运行顺序
...
  return gulp.src(['rev/**/*.json', htmlSrc]) // 这里的View/*.html是项目html文件路径，如果gulpfile.js和html文件同在一级目录下，修改为return;
    .pipe(revCollector())
...
  return gulp.src(['rev/{css,img,js}/*.json', 'src/*.html']) // 这里的View/*.html是项目html文件路径，如果gulpfile.js和html文件同在一级目录下，修改为return;
    .pipe(revCollector())
    .pipe(htmlmin(options)) // html压缩 
    .pipe(gulp.dest('dist/'));
});
...
gulp.task('watch', function() {
    gulp.watch([cssSrc, jsSrc, htmlSrc, 'js/*.js', '*.html'], ['dev']);
});

//开发构建
gulp.task('dev', function(cb) {
    runSequence(['revCss'], ['revJs'],['requirejs'],['csscompress'],['jscompress'],['imageMin'],['icon'],['revHtml'],['revRootHtml'],['replaceURL'],['watch'], cb);
});